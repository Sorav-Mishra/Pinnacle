...
//         .replace(/\s+/g, "-")
//         .replace(/[^\w-]/g, "");
//       router.push(`ssc-pyqs/${formattedName}`);
//     },
//     [router]
//   );
...
//       router.push(`ssc-pyqs/${formattedChapter}?subtopic=${formattedSubtopic}`);
//     }
//   };
...
const SSCIndex: React.FC = () => {
  ...
  return (
    <div className="p-3 sm:p-6 max-w-4xl mx-auto mt-4 sm:mt-16 text-gray-900 bg-white dark:bg-gray-900 dark:text-white min-h-screen transition-all">
      <h1 className="text-2xl sm:text-3xl font-bold text-center mb-4 sm:mb-6">
        {title}
      </h1>
      ...
      {!selectedChapter ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-6">
          {chapters.map((chapter) => (
            <div
              ...
              className="border border-gray-300 dark:border-gray-700 p-3 sm:p-5 rounded-lg cursor-pointer hover:bg-gray-100 hover:shadow-md dark:hover:bg-gray-800 transition-all text-center"
              aria-label={`${chapter.name} - ${chapter.questions} questions`}
            >
              <h2 className="text-base sm:text-lg font-semibold text-blue-600 dark:text-blue-400">
                {chapter.name}
              </h2>
        ...
        <div>
          <button
            className="mb-4 text-blue-600 underline flex items-center text-sm sm:text-base p-2 sm:p-0 dark:text-blue-400"
            onClick={() => setSelectedSubtopic(null)}
            aria-label="Back to topics"
          >
            <span className="mr-1">←</span> Back to {selectedChapter.name}{" "}
            ...
            {Array.isArray(selectedSubtopic.subtopics) &&
              selectedSubtopic.subtopics.map((sub) => (
                <div
                  ...
                  className="border border-gray-300 dark:border-gray-700 p-3 sm:p-4 rounded-lg cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-800 transition flex flex-col"
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) =>
                    e.key === "Enter" &&
                ...
                >
                  <h3 className="font-semibold text-blue-700 text-sm sm:text-base dark:text-blue-300">
                    {typeof sub === "string" ? sub : sub.name}
                  </h3>
                  {typeof sub !== "string" && (
